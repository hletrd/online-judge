...
/**
 * Bulk problem creator for algo.xylolabs.com — Advanced problems (DP, Graph, DS, etc.)
 * Usage: node scripts/algo-problems/runner-adv.mjs
 */
...
const BATCH_SIZE = 10;
const START_SEQ = 201; // continue after the first 200
...
async function main() {
  const { problems: b1 } = await import('./adv-batch-1.mjs');
  const { problems: b2 } = await import('./adv-batch-2.mjs');
  const { problems: b3 } = await import('./adv-batch-3.mjs');
  const { problems: b4 } = await import('./adv-batch-4.mjs');

  const all = [...b1, ...b2, ...b3, ...b4];
  console.log(`\n📦 Total advanced problems to create: ${all.length}\n`);
  ...
  for (let i = 0; i < all.length; i++) {
    ...
    try {
      ...
      ok++;
      console.log(`[${String(i + 1).padStart(3)}/${all.length}] ✅ ${p.title}`);
    } catch (e) {
      fail++;
      failures.push({ seq, title: p.title, error: e.message });
      console.error(`[${String(i + 1).padStart(3)}/${all.length}] ❌ ${p.title} — ${e.message}`);
    }
