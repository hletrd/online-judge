...
async function main() {
  const { problems: b1 } = await import('./story-batch-1.mjs');
  const { problems: b2 } = await import('./story-batch-2.mjs');
  const { problems: b3a } = await import('./story-batch-3a.mjs');
  const { problems: b3b } = await import('./story-batch-3b.mjs');
  const { problems: b4 } = await import('./story-batch-4.mjs');

  const all = [...b1, ...b2, ...b3a, ...b3b, ...b4];
  console.log(`\nTotal story problems: ${all.length}\n`);
